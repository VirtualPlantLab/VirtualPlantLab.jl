    ...
    meristem_rule = Rule(
        ...
        rhs = mer ->
            ...
            (
                TreeTypes.Bud(),
                TreeTypes.Leaf(
                    biomass = vleaf.biomass,
                    length = vleaf.length,
                    width = vleaf.width,
                ),
            ) +
            TreeTypes.Internode(
                biomass = vint.biomass,
                length = vint.length,
                width = vint.width,
            ) +
            TreeTypes.Meristem(),
...
function create_sky(; scene, lat = 52.0 * π / 180.0, DOY = 182)
    ...
    dome = sky(
        ...
        nphi = 12,
    ) # Number of discretization steps in the azimuth angle
...
function create_raytracer(scene, sources)
    ...
    RayTracer(scene, sources, settings = settings, acceleration = BVH, rule = SAH{3}(5, 10))
end
...
from its biomass
=#
function leaf_dims(biomass, data)
    leaf_biomass = biomass
    leaf_area = biomass / data.SLW
    leaf_length = sqrt(leaf_area * 4 * data.LS / pi)
    leaf_width = leaf_length / data.LS
    return leaf_length, leaf_width
end

function int_dims(biomass, data)
    int_biomass = biomass
    int_volume = biomass / data.SIW
    int_length = cbrt(int_volume * 4 * data.IS^2 / pi)
    int_width = int_length / data.IS
...
=#
sink_strength(leaf, data) =
    leaf.age > data.leaf_expansion ? 0.0 :
    pdf(leaf.sink, leaf.age / data.leaf_expansion) / 100.0
# plot(
#     0:1:50,
#     x -> sink_strength(TreeTypes.Leaf(age = x), TreeTypes.treeparams()),
#     xlabel = "Age",
#     ylabel = "Sink strength",
#     label = "Leaf",
# )
...
=#
function size_leaves!(all_leaves, tdata)
    for leaf in all_leaves
        leaf.length, leaf.width = leaf_dims(leaf.biomass, tdata)
    end
    return nothing
end
function size_internodes!(all_internodes, tdata)
    for int in all_internodes
        int.length, int.width = int_dims(int.biomass, tdata)
    end
    return nothing
end
...
function create_tree(origin, orientation, RUE)
    ...
    axiom =
        ...
        TreeTypes.Meristem()
    tree = Graph(axiom = axiom, rules = (meristem_rule, branch_rule), data = data)
    return tree
end
...
function VPL.feed!(turtle::Turtle, s::Soil, data)
    Rectangle!(
        ...
        length = s.length,
        width = s.width,
        color = RGB(255 / 255, 236 / 255, 179 / 255),
    )
end
soil_graph =
    RA(-90.0) +
    T(Vec(0.0, 10.0, 0.0)) + # Moves into position
...
for i = 1:5
    ...
    if mod(i, 5) == 0
        soil = Scene(Graph(axiom = soil_graph)) # Otherwise soil is converted into a mesh?
        render_forest(forest, soil)
    end
end
